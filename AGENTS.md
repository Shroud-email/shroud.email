...
- Style with Tailwind utility classes; see the JS & CSS Guidelines below for the Tailwind v4 setup
- Run `mix format` before committing
- Static analysis runs with `mix credo --strict` and `mix sobelow`—treat warnings as blockers

## JS & CSS Guidelines

- This project uses **Tailwind CSS v4**, which has **no `tailwind.config.js`**. Configuration lives in `assets/css/app.css` using the v4 directives already in place:

      @import 'tailwindcss';
      @plugin '@tailwindcss/forms';
      @plugin '@tailwindcss/typography';
      @source '../js';
      @source '../../lib/shroud_web';
